...
# --8<-- [start:heading] { #8-start-heading }
| 参数 | 类型 | 说明 |
|------|------|------|
# --8<-- [end:heading] { #8-end-heading }

# --8<-- [start:topP] { #8-start-topp }
| `topP` | Double | 也称为核采样。通过将概率值最高的标记添加到子集中，直到其概率总和达到指定的 `topP` 值，从而创建下一个标记的子集。取值范围大于 0.0 且小于等于 1.0。 |
# --8<-- [end:topP] { #8-end-topp }

# --8<-- [start:logprobs] { #8-start-logprobs }
| `logprobs` | Boolean | 如果设置为 `true`，则包含输出标记的对数概率。 |
# --8<-- [end:logprobs] { #8-end-logprobs }

# --8<-- [start:topLogprobs] { #8-start-toplogprobs }
| `topLogprobs` | Integer | 每个位置最可能的前 N 个标记数量。取值范围为 0–20。需要将 `logprobs` 参数设置为 `true`。 |
# --8<-- [end:topLogprobs] { #8-end-toplogprobs }

# --8<-- [start:frequencyPenalty] { #8-start-frequencypenalty }
| `frequencyPenalty` | Double | 对频繁出现的标记进行惩罚以减少重复。较高的 `frequencyPenalty` 值会导致措辞变化更大并减少重复。取值范围为 -2.0 到 2.0。 |
# --8<-- [end:frequencyPenalty] { #8-end-frequencypenalty }

# --8<-- [start:presencePenalty] { #8-start-presencepenalty }
| `presencePenalty` | Double | 防止模型重用已包含在输出中的标记。较高的值鼓励引入新标记和新主题。取值范围为 -2.0 到 2.0。 |
# --8<-- [end:presencePenalty] { #8-end-presencepenalty }

# --8<-- [start:stop] { #8-start-stop }
| `stop` | List&lt;String&gt; | 指示模型在遇到其中任何一个字符串时应停止生成内容的字符串。例如，要使模型在生成两个换行符时停止生成内容，可将停止序列指定为 `stop = listOf("/n/n")`。 |
# --8<-- [end:stop] { #8-end-stop }

# --8<-- [start:parallelToolCalls] { #8-start-paralleltoolcalls }
| `parallelToolCalls` | Boolean | 如果设置为 `true`，则可以并行运行多个工具调用。特别适用于自定义节点或代理策略之外的 LLM 交互。 |
# --8<-- [end:parallelToolCalls] { #8-end-paralleltoolcalls }

# --8<-- [start:promptCacheKey] { #8-start-promptcachekey }
| `promptCacheKey` | String | 用于提示缓存的稳定缓存键。OpenAI 使用它来缓存类似请求的响应。 |
# --8<-- [end:promptCacheKey] { #8-end-promptcachekey }

# --8<-- [start:safetyIdentifier] { #8-start-safetyidentifier }
| `safetyIdentifier` | String | 稳定且唯一的用户标识符，可用于检测违反 OpenAI 政策的用户。 |
# --8<-- [end:safetyIdentifier] { #8-end-safetyidentifier }

# --8<-- [start:serviceTier] { #8-start-servicetier }
...
# --8<-- [end:serviceTier] { #8-end-servicetier }

# --8<-- [start:store] { #8-start-store }
| `store` | Boolean | 如果设置为 `true`，提供商可能会存储输出以供后续检索。 |
# --8<-- [end:store] { #8-end-store }

# --8<-- [start:audio] { #8-start-audio }
...
# --8<-- [end:audio] { #8-end-audio }

# --8<-- [start:reasoningEffort] { #8-start-reasoningeffort }
...
# --8<-- [end:reasoningEffort]# --8<-- [start:webSearchOptions] { #8-end-reasoningeffort }
...
# --8<-- [end:webSearchOptions] { #8-start-websearchoptions }

# --8<-- [start:background] { #8-end-websearchoptions }
| `background` | Boolean | 在后台运行响应。 |
# --8<-- [end:background] { #8-start-background }

# --8<-- [start:include] { #8-end-background }
...
# --8<-- [end:include] { #8-start-include }

# --8<-- [start:maxToolCalls] { #8-end-include }
| `maxToolCalls` | Integer | 此响应中允许的内置工具调用的最大总数。取值需大于或等于 `0`。 |
# --8<-- [end:maxToolCalls] { #8-start-maxtoolcalls }

# --8<-- [start:reasoning] { #8-end-maxtoolcalls }
...
# --8<-- [end:reasoning] { #8-start-reasoning }

# --8<-- [start:truncation] { #8-end-reasoning }
...
# --8<-- [end:truncation] { #8-start-truncation }

# --8<-- [start:topK] { #8-end-truncation }
| `topK` | Integer | 生成输出时考虑的最高概率令牌数量。取值需大于或等于 0（具体提供方可能有最低值要求）。 |
# --8<-- [end:topK] { #8-start-topk }

# --8<-- [start:repetitionPenalty] { #8-end-topk }
...
# --8<-- [end:repetitionPenalty] { #8-start-repetitionpenalty }

# --8<-- [start:minP] { #8-end-repetitionpenalty }
| `minP` | Double | 过滤掉相对于最可能令牌的相对概率低于定义的 `minP` 值的令牌。取值范围为 0.0 至 0.1。 |
# --8<-- [end:minP] { #8-start-minp }

# --8<-- [start:topA] { #8-end-minp }
...
# --8<-- [end:topA] { #8-start-topa }

# --8<-- [start:transforms] { #8-end-topa }
...
# --8<-- [end:transforms]# --8<-- [start:models] { #8-start-transforms }
| `models` | List&lt;String&gt; | 请求允许使用的模型列表。 |
# --8<-- [end:models] { #8-end-transforms }

# --8<-- [start:route] { #8-start-models }
| `route` | String | 请求使用的路由策略。 |
# --8<-- [end:route] { #8-end-models }

# --8<-- [start:provider] { #8-start-route }
...
# --8<-- [end:provider] { #8-end-route }

# --8<-- [start:stopSequences] { #8-start-provider }
| `stopSequences` | List&lt;String&gt; | 导致模型停止生成内容的自定义文本序列。如果匹配，响应中 `stop_reason` 的值为 `stop_sequence`。 |
# --8<-- [end:stopSequences] { #8-end-provider }

# --8<-- [start:container] { #8-start-stopsequences }
...
# --8<-- [end:container] { #8-end-stopsequences }

# --8<-- [start:mcpServers] { #8-start-container }
...
# --8<-- [end:mcpServers] { #8-end-container }

# --8<-- [start:serviceTier] { #8-start-mcpservers }
...
# --8<-- [end:serviceTier] { #8-end-mcpservers }

# --8<-- [start:thinking] { #8-start-servicetier }
...
# --8<-- [end:thinking] { #8-end-servicetier }

# --8<-- [start:thinkingConfig] { #8-start-thinking }
...
# --8<-- [end:thinkingConfig] { #8-end-thinking }

# --8<-- [start:enableSearch] { #8-start-thinkingconfig }
...
# --8<-- [end:enableSearch] { #8-end-thinkingconfig }

# --8<-- [start:enableThinking] { #8-start-enablesearch }
...
# --8<-- [end:enableThinking]# --8<-- [start:randomSeed] { #8-end-enablesearch }
| `randomSeed` | 整数 | 用于随机采样的种子。若设置此值，相同参数与种子值的调用将产生确定性结果。 |
# --8<-- [end:randomSeed] { #8-start-enablethinking }

# --8<-- [start:promptMode] { #8-end-enablethinking }
...
# --8<-- [end:promptMode] { #8-start-randomseed }

# --8<-- [start:safePrompt] { #8-end-randomseed }
...
# --8<-- [end:safePrompt] { #8-start-promptmode }