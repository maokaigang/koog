...
| `eventId`       | String              | 是      |         | 事件或事件组的唯一标识符。                                                                                         |
| `executionInfo` | AgentExecutionInfo  | 是      |         | 提供与此事件关联的执行上下文信息。                                 |
| `agentId`       | String              | 是      |         | AI 智能体的唯一标识符。                                                                          |
| `runId`         | String              | 是      |         | AI 智能体运行实例的唯一标识符。                                                                      |
| `error`         | AIAgentError        | 是      |         | 智能体运行期间发生的具体错误。更多信息请参阅 [AIAgentError](#aiagenterror)。 |
...
<a id="aiagenterror"></a>
`AIAgentError` 类提供智能体运行期间发生错误的更多详细信息。包含以下字段：
...
`AgentExecutionInfo` 类提供执行路径的上下文信息，支持追踪智能体运行内的嵌套执行上下文。包含以下字段：
