...
该智能体使用默认的[历史记录提供者](index.md#history-providers)，该链接将对话历史存储在内存中。这意味着当应用程序退出时，历史记录会丢失。在实际应用中，您应该实现一个自定义的历史记录提供程序，以便将历史记录持久化存储在数据库或文件中。

`windowSize(20)` [预处理器](index.md#preprocessors) 确保了有限的上下文大小：智能体仅存储最多20条最新消息。若不如此，提示词的大小可能会超出上下文限制。
